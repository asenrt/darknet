...
data load_data_detection(load_args a)
{
    const int random_index = random_gen();
    int c = a.c ? a.c : 3;
    int use_mixup = a.mixup;
    int boxes = a.num_boxes;
    ...
    if (use_mixup == 3) {
        cut_x = (int*)calloc(a.n, sizeof(int));
        cut_y = (int*)calloc(a.n, sizeof(int));
        const float min_offset = 0.2; // 20%
        for (i = 0; i < a.n; ++i) {
            cut_x[i] = rand_int(a.w * min_offset, a.w * (1 - min_offset));
            cut_y[i] = rand_int(a.h * min_offset, a.h * (1 - min_offset));
        }
    }
    ...
    d.y = make_matrix(a.n, a.truth_size * boxes);
    int i_mixup = 0;
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        char** random_paths;
        if (a.track) random_paths = get_sequential_paths(a.paths, a.n, a.m, a.mini_batch, a.augment_speed, a.contrastive);
        else random_paths = get_random_paths_custom(a.paths, a.n, a.m, a.contrastive);

        for (i = 0; i < a.n; ++i) {
            float* truth = (float*)xcalloc(a.truth_size * boxes, sizeof(float));
            ...
            float resize_down = a.resize, resize_up = a.resize;
            ...
            if (!augmentation_calculated || !a.track)
            {
                augmentation_calculated = 1;
                resize_r1 = random_float();
                resize_r2 = random_float();

                if (!a.contrastive || a.contrastive_jit_flip || i % 2 == 0)
                {
                    r1 = random_float();
                    r2 = random_float();
                    r3 = random_float();
                    ...
                    flip = a.flip ? random_gen() % 2 : 0;
                }

                r_scale = random_float();

                if (!a.contrastive || a.contrastive_color || i % 2 == 0)
                {
                    dhue = rand_uniform_strong(-a.hue, a.hue);
                    dsat = rand_scale(a.saturation);
                    dexp = rand_scale(a.exposure);
                }
                ...
                if (a.gaussian_noise && rand_int(0, 1) == 1) gaussian_noise = a.gaussian_noise;
            ...
            if (a.letter_box)
            {
                ...
                float net_ar = (float)a.w / (float)a.h;
            ...
            int min_w_h = fill_truth_detection(filename, boxes, a.truth_size, truth, a.classes, flip, dx, dy, 1. / sx, 1. / sy, a.w, a.h);
            ...
            image ai = image_data_augmentation(src, a.w, a.h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                gaussian_noise, blur, boxes, a.truth_size, truth);

            if (use_mixup == 0) {
                d.X.vals[i] = ai.data;
                memcpy(d.y.vals[i], truth, a.truth_size * boxes * sizeof(float));
            }
            else if (use_mixup == 1) {
                if (i_mixup == 0) {
                    d.X.vals[i] = ai.data;
                    ...
                    image old_img = make_empty_image(a.w, a.h, c);
                    old_img.data = d.X.vals[i];
                    //show_image(ai, "new");
                    //show_image(old_img, "old");
                    //wait_until_press_key_cv();
                    blend_images_cv(ai, 0.5, old_img, 0.5);
                    blend_truth(d.y.vals[i], boxes, a.truth_size, truth);
                    free_image(old_img);
                    d.X.vals[i] = ai.data;
                }
            }
                ...
                if (i_mixup == 0) {
                    image tmp_img = make_image(a.w, a.h, c);
                ...
                const int left_shift = min_val_cmp(cut_x[i], max_val_cmp(0, (-pleft * a.w / ow)));
                const int top_shift = min_val_cmp(cut_y[i], max_val_cmp(0, (-ptop * a.h / oh)));

                const int right_shift = min_val_cmp((a.w - cut_x[i]), max_val_cmp(0, (-pright * a.w / ow)));
                const int bot_shift = min_val_cmp(a.h - cut_y[i], max_val_cmp(0, (-pbot * a.h / oh)));


                int k, x, y;
                for (k = 0; k < c; ++k) {
                    for (y = 0; y < a.h; ++y) {
                        int j = y * a.w + k * a.w * a.h;
                        if (i_mixup == 0 && y < cut_y[i]) {
                            int j_src = (a.w - cut_x[i] - right_shift) + (y + a.h - cut_y[i] - bot_shift) * a.w + k * a.w * a.h;
                            memcpy(&d.X.vals[i][j + 0], &ai.data[j_src], cut_x[i] * sizeof(float));
                        }
                        if (i_mixup == 1 && y < cut_y[i]) {
                            int j_src = left_shift + (y + a.h - cut_y[i] - bot_shift) * a.w + k * a.w * a.h;
                            memcpy(&d.X.vals[i][j + cut_x[i]], &ai.data[j_src], (a.w - cut_x[i]) * sizeof(float));
                        }
                        if (i_mixup == 2 && y >= cut_y[i]) {
                            int j_src = (a.w - cut_x[i] - right_shift) + (top_shift + y - cut_y[i]) * a.w + k * a.w * a.h;
                            memcpy(&d.X.vals[i][j + 0], &ai.data[j_src], cut_x[i] * sizeof(float));
                        }
                        if (i_mixup == 3 && y >= cut_y[i]) {
                            int j_src = left_shift + (top_shift + y - cut_y[i]) * a.w + k * a.w * a.h;
                            memcpy(&d.X.vals[i][j + cut_x[i]], &ai.data[j_src], (a.w - cut_x[i]) * sizeof(float));
                        }
                    }
                }

                blend_truth_mosaic(d.y.vals[i], boxes, a.truth_size, truth, a.w, a.h, cut_x[i], cut_y[i], i_mixup, left_shift, right_shift, top_shift, bot_shift, a.w, a.h, a.mosaic_bound);
            ...
            if (a.show_imgs && i_mixup == use_mixup)   // delete i_mixup
            {
                ...
                int t;
                for (t = 0; t < boxes; ++t) {
                    box b = float_to_box_stride(d.y.vals[i] + t * a.truth_size, 1);
                    if (!b.x) break;
...
data load_data_augment(load_args a)
{
    char** paths_stored = a.paths;
    if (a.m) a.paths = get_random_paths(a.paths, a.n, a.m);
    data d = { 0 };
    d.shallow = 0;
    d.X = load_image_augment_paths(a.paths, a.n, a.flip, a.min, a.max, a.w, a.h, a.angle, a.aspect, a.hue, a.saturation, a.exposure, a.dontuse_opencv, a.contrastive);
    d.y = load_labels_paths(a.paths, a.n, a.labels, a.classes, a.hierarchy, a.label_smooth_eps, a.contrastive);

    if (a.mixup && rand_int(0, 1)) {
        char** paths_mix = get_random_paths(paths_stored, a.n, a.m);
        data d2 = { 0 };
        d2.shallow = 0;
        d2.X = load_image_augment_paths(paths_mix, a.n, a.flip, a.min, a.max, a.w, a.h, a.angle, a.aspect, a.hue, a.saturation, a.exposure, a.dontuse_opencv, a.contrastive);
        d2.y = load_labels_paths(paths_mix, a.n, a.labels, a.classes, a.hierarchy, a.label_smooth_eps, a.contrastive);
        free(paths_mix);
        ...
        d4.shallow = 0;
        if (a.mixup >= 3) {
            char** paths_mix3 = get_random_paths(paths_stored, a.n, a.m);
            d3.X = load_image_augment_paths(paths_mix3, a.n, a.flip, a.min, a.max, a.w, a.h, a.angle, a.aspect, a.hue, a.saturation, a.exposure, a.dontuse_opencv, a.contrastive);
            d3.y = load_labels_paths(paths_mix3, a.n, a.labels, a.classes, a.hierarchy, a.label_smooth_eps, a.contrastive);
            free(paths_mix3);

            char** paths_mix4 = get_random_paths(paths_stored, a.n, a.m);
            d4.X = load_image_augment_paths(paths_mix4, a.n, a.flip, a.min, a.max, a.w, a.h, a.angle, a.aspect, a.hue, a.saturation, a.exposure, a.dontuse_opencv, a.contrastive);
            d4.y = load_labels_paths(paths_mix4, a.n, a.labels, a.classes, a.hierarchy, a.label_smooth_eps, a.contrastive);
            free(paths_mix4);
        }
        ...
        for (i = 0; i < d2.X.rows; ++i) {
            ...
            int mixup = a.mixup;
            if (a.mixup == 4) mixup = rand_int(2, 3); // alternate CutMix and Mosaic
            ...
            // CutMix -----------------------------------
                ...
                const int cut_w = rand_int(a.w * min, a.w * max);
                const int cut_h = rand_int(a.h * min, a.h * max);
                const int cut_x = rand_int(0, a.w - cut_w - 1);
                const int cut_y = rand_int(0, a.h - cut_h - 1);
                const int left = cut_x;
                const int right = cut_x + cut_w;
                const int top = cut_y;
                const int bot = cut_y + cut_h;

                assert(cut_x >= 0 && cut_x <= a.w);
                assert(cut_y >= 0 && cut_y <= a.h);
                assert(cut_w >= 0 && cut_w <= a.w);
                assert(cut_h >= 0 && cut_h <= a.h);

                assert(right >= 0 && right <= a.w);
                assert(bot >= 0 && bot <= a.h);

                assert(top <= bot);
                assert(left <= right);

                const float alpha = (float)(cut_w * cut_h) / (float)(a.w * a.h);
            ...
            {
                const float min_offset = 0.2; // 20%
                const int cut_x = rand_int(a.w * min_offset, a.w * (1 - min_offset));
                const int cut_y = rand_int(a.h * min_offset, a.h * (1 - min_offset));

                float s1 = (float)(cut_x * cut_y) / (a.w * a.h);
                float s2 = (float)((a.w - cut_x) * cut_y) / (a.w * a.h);
                float s3 = (float)(cut_x * (a.h - cut_y)) / (a.w * a.h);
                float s4 = (float)((a.w - cut_x) * (a.h - cut_y)) / (a.w * a.h);
    ...
    if (a.blur) {
        ...
        for (i = 0; i < d.X.rows; ++i) {
            if (random_gen() % 4 == 0) {
                image im = make_empty_image(a.w, a.h, 3);
                im.data = d.X.vals[i];
                int ksize = a.blur;
                if (a.blur == 1) ksize = 15;
    ...
    if (a.show_imgs) {
        ...
        for (i = 0; i < d.X.rows; ++i) {
            ...
            im.data = d.X.vals[i];
            char buff[1000];
            sprintf(buff, "aug_%d_%s_%d", i, basecfg((char*)a.paths[i]), random_gen());
            save_image(im, buff);
